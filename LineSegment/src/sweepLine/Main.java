...
class Segment {
	    ...
	    public Point first() { //left end point
	    ...
	    public Point second() { //right end point
	        if(p_1.get_x_coord() <= p_2.get_x_coord()) {
	            return p_2;
	        } else {
	            return p_1;
	    ...
	    public void calculate_value(double value) { //calculate y-coord when sweep line is at x  
	        double x1 = this.first().get_x_coord(); //using y-y1=m(x-x1)
...
class Event {
    ...
    Event(Point p, Segment s, int type) {
        this.point = p;
        this.segments = new ArrayList<>(Arrays.asList(s)); //crate copy of the original list of segments
...
class Intersection {
    ...
    Intersection(ArrayList<Segment> input_data) {
        this.Q = new PriorityQueue<>(new event_comparator()); //comparable interface
        this.T = new TreeSet<>(new segment_comparator());
        this.X = new ArrayList<>();
        for(Segment s : input_data) {
            this.Q.add(new Event(s.first(), s, 0)); //type=0 left endpoint 
            this.Q.add(new Event(s.second(), s, 1)); //type=1 right endpoint
        }
    }

    public void find_intersections() {
        while(!this.Q.isEmpty()) {
            Event e = this.Q.poll(); // fetch and remove the first element of the Queue
            double L = e.get_value(); //y-cord
            switch(e.get_type()) {
            case 0: //left endpoint event
